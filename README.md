...
- Copies Claude Code configurations from `claude/` to `~/.claude/` (CLAUDE.md, commands, skills), then symlinks the account-agnostic subset into every `~/.claude-account-<suffix>/`
- Each `~/.claude-account-<suffix>/` is an isolated config dir for an extra Claude account: run `claude-<suffix>` (alias sets `CLAUDE_CONFIG_DIR`) and log in once with that account. Only the per-account bits stay local (`.credentials.json`, `.claude.json`, `settings.json`, `history.jsonl`, `sessions/`); `projects/`, `skills/`, `rules/`, `CLAUDE.md` and `RTK.md` are symlinked into `~/.claude/`, so every account shares one set of instructions and skills and any account can `--resume` any conversation. Bootstrap also sets `disableRemoteControl: true` in each alt account's `settings.json`: resuming a conversation reconnects Remote Control from the record stored in the shared transcript, so an alt account resuming a main-account conversation would otherwise take that record over and the main account could no longer reconnect to the original remote session. Use `claude-<suffix>-rc` (lifts the setting for one invocation) when an alt account is wanted remotely on purpose. Aliases are discovered from disk, so to add an account just `mkdir ~/.claude-account-<suffix>`, open a new shell and run `claude-<suffix>` to log in; re-run `bootstrap.sh` to link the shared paths into it (bootstrap picks up every existing `~/.claude-account-*/` dir, and refuses to touch a `projects/` that already holds transcripts — merge those with `claude-share --apply` first). Edit `CLAUDE_ALT_SUFFIXES` in `bootstrap.sh` only to change the accounts created by default
...
├── README.md            # This file
├── bin/                 # Utility scripts
│   ├── claude-share     # Merge existing Claude account dirs onto the shared ~/.claude layout
